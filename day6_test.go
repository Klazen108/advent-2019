...
func TestDay6_1(t *testing.T) {
	result := Challenge6_1(GetChallengeData())
	if result != "402879" {
...
func TestDay6_2(t *testing.T) {
	result := Challenge6_2(GetChallengeData())
	if result != "484" {
		t.Errorf("Result was incorrect, got: %s, want: %s.", result, "484")
	}
}

func TestDay6_2LCA(t *testing.T) {
	satMap := ParseMapData(GetChallengeData())

	youAncestry := satMap.GetAdd("YOU").GetAncestry()
	sanAncestry := satMap.GetAdd("SAN").GetAncestry()
	lca, xfer := FindLCA(youAncestry, sanAncestry)

	if lca.name != "DXC" {
		t.Errorf("Result was incorrect, got: %s, want: %s.", lca.name, "DXC")
	}
	if xfer != 486 {
		t.Errorf("Result was incorrect, got: %d, want: %d.", xfer, 486)
	}
}

func TestDay6_1Example(t *testing.T) {
	result := Challenge6_1(`COM)B
	B)C
	C)D
	D)E
	E)F
...
func TestDay6_2Example(t *testing.T) {
	result := Challenge6_2(`COM)B
	B)C
	C)D
	D)E
	E)F
...
func TestDay6_2Example2(t *testing.T) {
	result := Challenge6_2(`COM)B
	B)C
	C)SAN
	B)G
...
func TestDay6_2Example3(t *testing.T) {
	result := Challenge6_2(`COM)B
	B)C
	C)D
	D)SAN
	...
	G)YOU`)
	if result != "3" {
		t.Errorf("Result was incorrect, got: %s, want: %s.", result, "3")
	}
}
...
func TestFindLCA(t *testing.T) {
	...
	lca, _ := FindLCA(youAncestry, sanAncestry)

	if lca == nil {
		t.Errorf("Result was nil, expected match")
	}
	if lca.name != "D" {
		t.Errorf("Result was incorrect, got: %s, want: %s.", lca.name, "D")
	}
}
...
func TestGetAdd(t *testing.T) {
	...
	object := satMap.GetAdd("E")
	if object == nil {
		t.Errorf("Object should not be nil")
	}
}

func TestCountsIndirectOrbits(t *testing.T) {
	satMap := SatMap(make(map[string]*Object))
	a := satMap.GetAdd("A")
	b := satMap.GetAdd("B")
	c := satMap.GetAdd("C")
	...
	a.satellites = append(a.satellites, b)
